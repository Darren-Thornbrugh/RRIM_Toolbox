...
def _gdal_type_from_string(s):
    s=s.upper()
    if s=="FLOAT32":return gdal.GDT_Float32
    if s=="FLOAT64":return gdal.GDT_Float64
    if s=="INT16":return gdal.GDT_Int16
    if s=="UINT16":return gdal.GDT_UInt16
    if s=="BYTE":return gdal.GDT_Byte
    return gdal.GDT_Float32


def _gdal_creation_options(compression, predictor=None, tiled=True, zlevel=None):
    opts = []
    if tiled:
        opts.append("TILED=YES")
    if compression and compression.upper() != "NONE":
        comp = compression.upper()
        opts.append(f"COMPRESS={comp}")
        if predictor is not None and comp in ("LZW", "DEFLATE"):
            opts.append(f"PREDICTOR={predictor}")
        if zlevel is not None and comp == "DEFLATE":
            opts.append(f"ZLEVEL={int(zlevel)}")
    return opts
...
# ------------------------------------------------------------------------------
# NativeTopographicOpenness – ArcGIS Pro–safe reimplementation
#
...
def _horizon_shift_vector_cached(ndirs,radius,min_radius=1):
    ...
    for i in range(ndirs):
        ...
        pairs=np.stack((yi,xi),axis=1)
        pairs=np.unique(pairs,axis=0)

        # ensure valid (dy,dx) table
...
def _compute_openness_pos_neg_fast(dem_arr,radius,ndirs,out_nodata,in_nodata=None):
    ...
    for shifts,inv_dists in dirs:
        ...
        for k in range(shifts.shape[0]):
            dy=int(shifts[k,0]);dx=int(shifts[k,1]);inv=inv_dists[k]

            y0=pad-dy; x0=pad-dx
            y1=y0+h;  x1=x0+w

            if y0<0 or x0<0 or y1>dem_pad.shape[0] or x1>dem_pad.shape[1]:
                continue
        ...
        np.add(pos_sum,max_pos,out=pos_sum)
        np.add(neg_sum,max_neg,out=neg_sum)

    scale=np.float32(1.0/ndirs);half_pi=np.float32(0.5*np.pi)

    np.multiply(pos_sum,scale,out=pos_sum)
    np.subtract(half_pi,pos_sum,out=pos_sum)
    np.degrees(pos_sum,out=pos_sum)

    np.multiply(neg_sum,scale,out=neg_sum)
    np.subtract(half_pi,neg_sum,out=neg_sum)
...
class NativeTopographicOpenness(object):
    def __init__(self):
        self.label="1. Native Topographic Openness (POS/NEG)"
        self.description="Computes positive and negative topographic openness."
    ...
    def execute(self,p,msg):
        ...
        if folder:
            for f in os.listdir(folder):
                if f.lower().endswith((".tif",".tiff",".img")):
                    dem_list.append(os.path.join(folder,f))
        ...
        def process(dem_input):
            try:
                ...
                pos_out=os.path.join(out,f"{base}_pos.tif")
                neg_out=os.path.join(out,f"{base}_neg.tif")
                ...
                for path,data in[(pos_out,pos),(neg_out,neg)]:
                    ...
                    b=d.GetRasterBand(1)
                    b.WriteArray(data);b.SetNoDataValue(nodata);b.FlushCache()
                    d.FlushCache();d=None
                ...
                outputs.append(pos_out);outputs.append(neg_out)

                return(f"{base}: read {t_read-t0:.2f}s, compute {t_compute-t_read:.2f}s, write {t_write-t_compute:.2f}s, total {t_write-t0:.2f}s")
...
class OpennessIndex(object):
    def __init__(self):
        self.label = "2. Openness Index (POS–NEG)"
        self.description = "Computes openness index from POS and NEG rasters: (pos - neg) / 2."
        self.canRunInBackground = False

    def getParameterInfo(self):
        p_pos = arcpy.Parameter(displayName="Positive Openness Raster (POS)",name="pos_raster",datatype="GPRasterLayer",parameterType="Optional",direction="Input")
        p_neg = arcpy.Parameter(displayName="Negative Openness Raster (NEG)",name="neg_raster",datatype="GPRasterLayer",parameterType="Optional",direction="Input")
        p_folder = arcpy.Parameter(displayName="Input Folder (optional, POS/NEG tiles)",name="in_folder",datatype="DEFolder",parameterType="Optional",direction="Input")
        p_out = arcpy.Parameter(displayName="Output Folder",name="out_folder",datatype="DEWorkspace",parameterType="Required",direction="Output")
        ...
        p_nd.value = -9999.0
        p_comp = arcpy.Parameter(displayName="Output Compression",name="out_compression",datatype="GPString",parameterType="Required",direction="Input")
        p_comp.filter.type="ValueList";p_comp.filter.list=["DEFLATE"];p_comp.value="DEFLATE"
        return [p_pos,p_neg,p_folder,p_out,p_nd,p_comp]

    def _add_to_map(self,r):
        try:
            aprx=arcpy.mp.ArcGISProject("CURRENT");m=aprx.activeMap;m.addDataFromPath(r)
        except:pass

    def _compute_open(self,pos_path,neg_path,out_path,nodata,comp):
        pos_ds=gdal.Open(pos_path);neg_ds=gdal.Open(neg_path)
        if pos_ds is None or neg_ds is None:raise arcpy.ExecuteError("Could not open POS or NEG raster.")
        pos=pos_ds.GetRasterBand(1).ReadAsArray().astype(np.float32)
        neg=neg_ds.GetRasterBand(1).ReadAsArray().astype(np.float32)
        pos_nd=pos_ds.GetRasterBand(1).GetNoDataValue();neg_nd=neg_ds.GetRasterBand(1).GetNoDataValue()
        mask=np.zeros_like(pos,bool)
        if pos_nd is not None:mask|=(pos==pos_nd)
        if neg_nd is not None:mask|=(neg==neg_nd)
        pos=np.where(~mask,pos,np.nan);neg=np.where(~mask,neg,np.nan)
        out=(pos-neg)/2.0;out[~np.isfinite(out)]=nodata
        driver=gdal.GetDriverByName("GTiff");rows,cols=out.shape
        opts=["COMPRESS=DEFLATE","PREDICTOR=2","TILED=YES"]
        ds=driver.Create(out_path,cols,rows,1,gdal.GDT_Float32,options=opts)
        ds.SetGeoTransform(pos_ds.GetGeoTransform());ds.SetProjection(pos_ds.GetProjection())
        b=ds.GetRasterBand(1);b.WriteArray(out);b.SetNoDataValue(nodata)
        ds.FlushCache();ds=None

    def execute(self,p,msg):
        pos=p[0].value;neg=p[1].value;folder=p[2].valueAsText;out=p[3].valueAsText;nodata=float(p[4].value);comp=p[5].valueAsText
        if not os.path.exists(out):os.makedirs(out)
        if folder:
            ...
            for f in files:
                low=f.lower()
                if "_pos" in low:pos_d[os.path.splitext(low.replace("_pos",""))[0]]=os.path.join(folder,f)
                elif "_neg" in low:neg_d[os.path.splitext(low.replace("_neg",""))[0]]=os.path.join(folder,f)
            common=sorted(set(pos_d)&set(neg_d))
            if not common:raise arcpy.ExecuteError("No matching POS/NEG pairs found.")
            for k in common:
                pos_path=pos_d[k];neg_path=neg_d[k];out_path=os.path.join(out,f"{k}_open.tif")
                msg.addMessage(f"Computing openness index for: {k}")
                self._compute_open(pos_path,neg_path,out_path,nodata,comp)
            return
        if not pos or not neg:raise arcpy.ExecuteError("Provide POS/NEG rasters or folder.")
        pos_path=arcpy.Describe(pos).catalogPath;neg_path=arcpy.Describe(neg).catalogPath
        base=os.path.splitext(os.path.basename(pos_path))[0]
        if base.lower().endswith("_pos"):base=base[:-4]
        out_path=os.path.join(out,f"{base}_open.tif")
        msg.addMessage(f"Computing openness index for: {base}")
        self._compute_open(pos_path,neg_path,out_path,nodata,comp)
...
class SlopeFromDEM(object):
    def __init__(self):
        self.label="3. Slope from DEM"
        self.description="Computes slope (degrees) from DEM using fast GDAL/NumPy Horn method."
        self.canRunInBackground=False
    ...
    def _compute_slope(self,arr,gt,nodata):
        ...
        dzdx=(pad[1:-1,2:] - pad[1:-1,:-2])/(2*xres)
        dzdy=(pad[2:,1:-1] - pad[:-2,1:-1])/(2*yres)

        slope=np.degrees(np.arctan(np.sqrt(dzdx*dzdx + dzdy*dzdy)))
        slope[np.isnan(dem)] = nodata
        return slope
    ...
    def execute(self,p,msg):
        dem=p[0].value;folder=p[1].valueAsText;out=p[2].valueAsText;nodata=float(p[3].value)
        if not os.path.exists(out):os.makedirs(out)
        ...
        if folder:
            for f in os.listdir(folder):
                if f.lower().endswith((".tif",".img")):process(os.path.join(folder,f))
            return
...
class RedReliefImageMap(object):
    def __init__(self):
        self.label = "4. Red Relief Image Map (RRIM)"
        self.description = "RRIM: Cividis openness + red-tinted slope overlay (true RGB)."
        self.canRunInBackground = False

    def getParameterInfo(self):
        p1 = arcpy.Parameter(
            displayName="Slope Raster (single-tile mode)",
            name="in_slope",
        ...
        p2 = arcpy.Parameter(
            displayName="Openness Raster (single-tile mode)",
            name="in_open",
            datatype="GPRasterLayer",
            parameterType="Optional",
            direction="Input"
        ...
        p3 = arcpy.Parameter(
            displayName="Slope Folder (optional, batch mode)",
            name="slope_folder",
            datatype="DEFolder",
            parameterType="Optional",
            direction="Input"
        ...
        p4 = arcpy.Parameter(
            displayName="Openness Folder (optional, batch mode)",
            name="open_folder",
            datatype="DEFolder",
            parameterType="Optional",
            direction="Input"
        ...
        p6 = arcpy.Parameter(
            displayName="Output NoData value (RGB)",
            name="out_nodata",
            datatype="GPLong",
            parameterType="Optional",
            direction="Input"
    ...
    def _compute_rrim_core(self, slope_path, open_path):
        s_ds = gdal.Open(slope_path)
        o_ds = gdal.Open(open_path)
        s = s_ds.GetRasterBand(1).ReadAsArray().astype(np.float32)
        o = o_ds.GetRasterBand(1).ReadAsArray().astype(np.float32)
        sn = s_ds.GetRasterBand(1).GetNoDataValue()
        on = o_ds.GetRasterBand(1).GetNoDataValue()
        ...
        o = np.where(~m, o, np.nan)
        ov = o[np.isfinite(o)]
        sv = s[np.isfinite(s)]
        omin = float(ov.mean() - 3.5 * ov.std())
        omax = float(ov.mean() + 3.5 * ov.std())
        smin = max(0.0, float(sv.mean() - 4 * sv.std()))
        smax = float(sv.mean() + 4 * sv.std())
        onorm = np.clip((o - omin) / (omax - omin), 0, 1)
        snorm = np.clip((s - smin) / (smax - smin), 0, 1)
        onorm[~np.isfinite(onorm)] = 0
        snorm[~np.isfinite(snorm)] = 0
        oi = (onorm * 255).astype(np.uint8)
        si = (snorm * 255).astype(np.uint8)
        c = CIVIDIS_LUT[oi]
        r = REDS_LUT[si]
        try:
            b = _blend_rgb(c.astype(np.float32), r.astype(np.float32), 0.7).astype(np.uint8)
        except:
            b = ((1 - 0.7) * c + 0.7 * r).clip(0, 255).astype(np.uint8)
    ...
    def _save_tif(self, R, G, B, src_ds, out_path, nd):
        ...
        ds.GetRasterBand(2).SetNoDataValue(nd)
        ds.GetRasterBand(3).SetNoDataValue(nd)
        ds.FlushCache()
        ds = None

    def _add_tif_as_exact_rgb_no_stretch(self, tif_path):
        tif_path = os.path.abspath(tif_path)
        if not arcpy.Exists(tif_path):
            raise FileNotFoundError(tif_path)
        aprx = arcpy.mp.ArcGISProject("CURRENT")
        m = aprx.activeMap
        if m is None:
            raise RuntimeError("No active map found in ArcGIS Pro.")
        lyr = m.addDataFromPath(tif_path)
        if not lyr.isRasterLayer:
            raise RuntimeError("Added layer is not a raster layer.")
        sym = lyr.symbology
        try:
            sym.updateColorizer("RasterRGBColorizer")
            cz = sym.colorizer
            if hasattr(cz, "redBand"):
                cz.redBand = "Band_1"
            if hasattr(cz, "greenBand"):
                cz.greenBand = "Band_2"
            if hasattr(cz, "blueBand"):
                cz.blueBand = "Band_3"
            ...
            if hasattr(cz, "gamma"):
                try:
                    ...
                    pass
            if hasattr(cz, "useGamma"):
                try:
                    cz.useGamma = False
                except Exception:
                    pass
            for dra_prop in ["useDRA", "DRA", "dynamicRangeAdjustment", "useDynamicRangeAdjustment"]:
                if hasattr(cz, dra_prop):
                    try:
                        setattr(cz, dra_prop, False)
                    except Exception:
                        pass
            lyr.symbology = sym
        except Exception:
        ...
        try:
            ...
            if hasattr(cim, "colorizer") and cim.colorizer:
                c = cim.colorizer
                if hasattr(c, "stretchType"):
                    c.stretchType = "None"
                for dra_prop in ["useDRA", "DRA", "dynamicRangeAdjustment", "useDynamicRangeAdjustment"]:
                    if hasattr(c, dra_prop):
                        try:
                            setattr(c, dra_prop, False)
                        except Exception:
                            pass
                if hasattr(c, "gamma"):
                    try:
    ...
    def execute(self, p, msg):
        ...
        if sf and of:
            ...
            sd, od = {}, {}
            for f in sl:
                if "_slope" in f.lower():
                    sd[os.path.splitext(f.lower().replace("_slope", ""))[0]] = os.path.join(sf, f)
            for f in ol:
                if "_open" in f.lower():
                    od[os.path.splitext(f.lower().replace("_open", ""))[0]] = os.path.join(of, f)
...
class RedReliefImageMapClassic(object):
    def __init__(self):
        self.label = "5. Red Relief Image Map (Classic)"
    ...
    def _compute_rrim_classic_core(self, slope_path, open_path):
        ...
        sv = s[np.isfinite(s)]
        ov = o[np.isfinite(o)]
        smin = float(np.percentile(sv, 0.5))
        smax = float(np.percentile(sv, 99.5))
        omin = float(np.percentile(ov, 0.5))
        omax = float(np.percentile(ov, 99.5))
        ...
        r = REDS_LUT[si]
        try:
            b = _blend_rgb(g.astype(np.float32), r.astype(np.float32), 0.7).astype(np.uint8)
        except Exception:
            b = ((1 - 0.7) * g + 0.7 * r).clip(0, 255).astype(np.uint8)
        return b[:, :, 0], b[:, :, 1], b[:, :, 2], s_ds
